"""Deterministically seed fixtures/pager_repo as a git repo.
...
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "pager_repo"
...
def ensure_seeded_pager(fixture_dir: Path = FIXTURE_DIR) -> str:
    """Create/commit the fixture repo if needed; return HEAD SHA."""
    if not (fixture_dir / "store/paging.py").exists():
    ...
    if not git_dir.exists():
        ...
        _git("commit", "-m", "seed: pager_repo v1", cwd=fixture_dir)
