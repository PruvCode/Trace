"""Deterministically seed fixtures/creds_repo as a git repo.
...
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "creds_repo"
...
def ensure_seeded_creds(fixture_dir: Path = FIXTURE_DIR) -> str:
    """Create/commit the fixture repo if needed; return HEAD SHA."""
    if not (fixture_dir / "auth" / "login.py").exists():
    ...
    if not git_dir.exists():
        ...
        _git("commit", "-m", "seed: creds_repo v1", cwd=fixture_dir)
