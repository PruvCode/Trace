"""Deterministically seed fixtures/queue_repo as a git repo.
...
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "queue_repo"
...
def ensure_seeded_queue(fixture_dir: Path = FIXTURE_DIR) -> str:
    """Create/commit the fixture repo if needed; return HEAD SHA."""
    if not (fixture_dir / "queue/worker.py").exists():
    ...
    if not git_dir.exists():
        ...
        _git("commit", "-m", "seed: queue_repo v1", cwd=fixture_dir)
