"""Deterministically seed fixtures/data_repo as a git repo.
...
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "data_repo"
...
def ensure_seeded_data(fixture_dir: Path = FIXTURE_DIR) -> str:
    """Create/commit the fixture repo if needed; return HEAD SHA."""
    if not (fixture_dir / "ingest/csv_loader.py").exists():
    ...
    if not git_dir.exists():
        ...
        _git("commit", "-m", "seed: data_repo v1", cwd=fixture_dir)
