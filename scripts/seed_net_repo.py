"""Deterministically seed fixtures/net_repo as a git repo.
...
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "net_repo"
...
def ensure_seeded_net(fixture_dir: Path = FIXTURE_DIR) -> str:
    """Create/commit the fixture repo if needed; return HEAD SHA."""
    if not (fixture_dir / "net/retry.py").exists():
    ...
    if not git_dir.exists():
        ...
        _git("commit", "-m", "seed: net_repo v1", cwd=fixture_dir)
