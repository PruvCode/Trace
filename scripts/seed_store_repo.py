"""Deterministically seed fixtures/store_repo as a git repo.
...
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "store_repo"
...
SEED_ENV = {
    ...
    "GIT_AUTHOR_DATE": "2027-01-01T00:00:00+00:00",
...
def ensure_seeded_store(fixture_dir: Path = FIXTURE_DIR) -> str:
    """Create/commit the fixture repo if needed; return HEAD SHA."""
    if not (fixture_dir / "migrate/backfill.py").exists():
    ...
    if not git_dir.exists():
        ...
        _git("commit", "-m", "seed: store_repo v1", cwd=fixture_dir)
